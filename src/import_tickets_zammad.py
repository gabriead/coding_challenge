...
class TicketCreator:
    ...
    def download_ds(self, path="consumer_complaints_ds"):
        ...
        if not os.path.isdir(path):
            ...
            logging.info("Dataset is already locally available, no download needed")
    ...
    def load_training_samples(self, num_samples: int) -> pd.DataFrame:
        dataset_hub = load_from_disk("consumer_complaints_ds")
    ...
    def create_tickets_from_samples(self, samples: pd.DataFrame, ticket_api: TicketAPI):
        ...
        logging.info("Creating tickets within Zammad")
